...
if(requery.circuit=="Men" && requery.year=="2020" && requery.sets=="All"){
    ...
    if(requery.result=="Win"){
        ...
        const res=  knex("T_ATP_2020_65").select(...required)
        .where({ "winner_name":requery.player_name ,
        "surface":requery.surface });
        return res;
    }
    else if(requery.result=="Loss"){
        const res= knex("T_ATP_2020_65").select(...required).where({"loser_name":requery.player_name,
        "surface":requery.surface});
        return res;
        }
...
if(requery.result=="Win"){
    ...
    const res=  knex("T_WTA_2020_65").select(...required)
    .where({ "winner_name":requery.player_name ,
    "surface":requery.surface });
    return res;
}
else if(requery.result=="Loss"){
    const res= knex("T_WTA_2020_65").select(...required).where({"loser_name":requery.player_name,
    "surface":requery.surface});
    return res;
    }
